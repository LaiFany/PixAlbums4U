		...
		<section class = "usermainpage-section">
         ...
         <div class="col-md-8">
				...
				<?php
					...
					$query = mysqli_query($mysqli,"SELECT id, url, description, member_id, album_id, name FROM image ORDER BY date_time_created DESC");
					if(empty($query)){
               ...
               <?php
                  ...
                  if(empty($query3)){
                     ...
                     while($run3 = mysqli_fetch_array($query3)){
                        $url = $run3['url'];
               ?>
               <img src = "uploads/<?php echo $url; ?>" width = "100" height = "100">
               <?php
                  }
               }
               ?>
            </div>
            <div class = "oldestphoto">
               <a href = "oldestphotopage.php"><h1>Oldest Photos</h1></a>
               <?php
                  //display a group of 6 images sorted by their creation date and time.
                  $query3 = mysqli_query($mysqli,"SELECT url FROM image ORDER BY date_time_created LIMIT 6");
                  if(empty($query3)){
                     echo 'No images to display!';
                  }
                  else{
