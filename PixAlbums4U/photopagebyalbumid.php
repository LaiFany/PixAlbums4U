...
<html>
	...
	<body onresize="changePaddingTop()" onkeydown="kbdViewPhotos(arguments[0])">
      ...
      <script>changePaddingTop();</script>
			...
			<?php
				//if next button is clicked, the photoslider class retrives information loads the next picture sorted random from the album, and the photopageinfo class displays the retrieved information.
				if(isset($_GET['previous_id'])){
					$previous_id = $_GET['previous_id'];
					$query = mysqli_query($mysqli,"SELECT album_id FROM image WHERE id = '$previous_id'");
					$run = mysqli_fetch_assoc($query);
					$album_id = $run['album_id'];
					$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id > '$previous_id' AND album_id = '$album_id' ORDER BY id LIMIT 1");
					if(mysqli_num_rows($query) != 0){
						...
						?>
						<div class = "prev">
							<a href = "photopagebyalbumid.php?next_id=<?php echo $id;?>" ><span class="glyphicon glyphicon-arrow-left"></span></a>
						</div>
						<div class = "next">
							<a href = "photopagebyalbumid.php?previous_id=<?php echo $id;?>" ><span class="glyphicon glyphicon-arrow-right"></span></a>
						</div>
						</div>
			</div>
			...
			<div class = "photoslider">
				...
				//if previous button is clicked, the photoslider class retrives information loads the previous picture sorted by ID, and the photopageinfo class displays the retrieved information.
					...
					$query = mysqli_query($mysqli,"SELECT album_id FROM image WHERE id = '$next_id'");
					$run = mysqli_fetch_assoc($query);
					$album_id = $run['album_id'];
					$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id < '$next_id' AND album_id = '$album_id' ORDER BY id DESC LIMIT 1");
					if(mysqli_num_rows($query) != 0){
						while($run = mysqli_fetch_assoc($query)){
							$id = $run['id'];
							$url = $run['url'];
			...
			<div class = "photoslider">
					...
					//if reached the beginning of the slide, the photoslider class retrives information,  and loads the same picture, but only displays the next button, and the photopageinfo class displays the retrieved information.
						...
						$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id = '$next_id' AND album_id = '$album_id' ORDER BY id LIMIT 1");
						if(mysqli_num_rows($query) != 0){
							while($run = mysqli_fetch_assoc($query)){
								$id = $run['id'];
								$url = $run['url'];
