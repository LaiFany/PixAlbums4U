      ...
      <script>changePaddingTop();</script>
			...
			<?php
				...
				if(isset($_GET['previous_id'])){
					...
					$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id < '$previous_id' ORDER BY id DESC LIMIT 1");
					if(mysqli_num_rows($query) != 0){
						...
						?>
						<div class = "prev">
							<a href = "photopagebyoldest.php?next_id=<?php echo $id;?>" ><span class="glyphicon glyphicon-arrow-left"></span></a>
						</div>
						<div class = "next">
							<a href = "photopagebyoldest.php?previous_id=<?php echo $id;?>" ><span class="glyphicon glyphicon-arrow-right"></span></a>
						</div>
						</div>
			</div>
			...
			<?php
					...
					//if reached the end of the slide, the photoslider class retrives information,  and loads the same picture, but only displays the previous button, and the photopageinfo class displays the retrieved information.
						...
						$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id = '$previous_id' LIMIT 1");
						if(mysqli_num_rows($query) != 0){
							while($run = mysqli_fetch_assoc($query)){
								$id = $run['id'];
								$url = $run['url'];
			...
			<div class = "photoslider">
				...
				//if previous button is clicked, the photoslider class retrives information loads the previous picture sorted by ID, and the photopageinfo class displays the retrieved information.
					...
					$query = mysqli_query($mysqli, "SELECT * FROM image WHERE id > '$next_id' LIMIT 1");
					if(mysqli_num_rows($query) != 0){
						while($run = mysqli_fetch_assoc($query)){
							$id = $run['id'];
							$url = $run['url'];
